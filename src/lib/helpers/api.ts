...
export const fetchWithdrawalFee = async (
  asset_id: string,
  destination: string,
  tag: string,
) => {
...
export const fetchAssets = async (user: User | undefined) => {
  ...
  const [allAssets, addresses, ethBalance, tokens] = await Promise.all([
    (async () => {
      ...
      const restAssets = await Promise.all(
        rest.map((id) => networkClient.fetchAsset(id)),
      );
  ...
  let assets = allAssets.filter(({ asset_id }) =>
    WHITELIST_ASSET_ID.includes(asset_id)
  );
  ...
  let whiteListAssets = assets.map((a) => {
    ...
    const token = tokens?.find(
      (token) =>
        token.contractAddress.toLowerCase() === asset.contract?.toLowerCase(),
    );
    if (!token) {
      asset.balance = "0";
      return asset;
  ...
  // chain
  const chainIds = [
    ...new Set(whiteListAssets.map(({ chain_id }) => chain_id)),
  ];
...
export const checkOrder = async (
  ...
  let counter = 0;
  const fetchOrderStatus = source === "4Swap"
    ? fetch4SwapOrder
    : fetchMixPayOrder;
  ...
  return new Promise((resolve, reject) => {
    const timer = setInterval(async () => {
      ...
      try {
        ...
        if (
          (source === "4Swap" && res &&
            (res as OrderResponse).state === "Done") ||
          (source === "MixPay" && res &&
            (res as MixPayPaymentResponse).data.status === "success")
        ) {
          clearInterval(timer);
          resolve(true);
        }
